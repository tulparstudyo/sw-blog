	...
	implements Iface, \Aimeos\Controller\Frontend\Common\Iface
{
	...
	public function category( $catIds, string $listtype = 'default', int $level = \Aimeos\MW\Tree\Manager\Base::LEVEL_ONE ) : Iface
	{
		if( !empty( $catIds ) && ( $ids = $this->validateIds( (array) $catIds ) ) !== [] )
		{
			...
			$this->sort[] = $this->filter->sort( '+', 'post.id' ); // prevent flaky order if posts have same position
		}
	...
	{
		return $this->manager->findItem( $code, $this->domains, 'post', null, true );
	}
	...
	public function has( string $domain, string $type = null, string $refId = null ) : Iface
	{
		...
		$func = $this->filter->createFunction( 'post:has', $params );
		$this->conditions[] = $this->filter->compare( '!=', $func, null );
		return $this;
	}
	...
	public function post( $prodIds ) : Iface
	{
		if( !empty( $prodIds ) && ( $ids = array_unique( $this->validateIds( (array) $prodIds ) ) ) !== [] ) {
			$this->conditions[] = $this->filter->compare( '==', 'post.id', $ids );
		}
	...
	{
		$func = $this->filter->createFunction( 'post:prop', [$type, $langid, $value] );
		$this->conditions[] = $this->filter->compare( '!=', $func, null );
		return $this;
	}
	...
	public function sort( string $key = null ) : Iface
	{
		...
		foreach( $list as $sortkey )
		{
			...
			switch( $sortkey )
			{
				...
				case 'code':
					$this->sort[] = $this->filter->sort( $direction, 'post.code' );
					break;

				case 'ctime':
					$this->sort[] = $this->filter->sort( $direction, 'post.ctime' );
					break;
