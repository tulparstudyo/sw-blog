...
/** admin/jqadm/swpost/item/price/config/suggest
 * List of suggested configuration keys in swpost price panel
 *
				...
				<div v-for="(item, idx) in items" v-bind:key="idx" class="group-item card">
					...
					<div v-bind:id="'item-price-group-item-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
						...
						<div class="card-tools-right">
							<div v-if="item['swpost.lists.siteid'] == siteid && !item['_nosort']"
								class="btn btn-card-header act-move fa" tabindex="<?= $this->get( 'tabindex' ); ?>"
								title="<?= $enc->attr( $this->translate( 'admin', 'Move this entry up/down' ) ); ?>">
							</div>
							<div v-if="item['swpost.lists.siteid'] == siteid"
					...
					<div v-bind:id="'item-price-group-data-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
						...
						<div v-show="item['_ext']" class="col-xl-6 content-block secondary">
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select is="select-component" required class="form-control custom-select listitem-type" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ); ?>"
											v-bind:items="JSON.parse('<?= $enc->attr( $listTypes->col( 'swpost.lists.type.label', 'swpost.lists.type.code' )->toArray() ) ?>')"
											v-bind:name="'<?= $enc->attr( $this->formparam( ['price', 'idx', 'swpost.lists.type'] ) ); ?>'.replace('idx', idx)"
											v-bind:text="'<?= $enc->html( $this->translate( 'admin', 'Please select' ) ); ?>'"
											v-bind:readonly="item['swpost.lists.siteid'] != siteid"
											v-model="item['swpost.lists.type']" >
										</select>
									</div>
									<div class="col-sm-12 form-text text-muted help-text">
							...
							<?php else : ?>
								<input class="listitem-type" type="hidden"
									v-bind:name="'<?= $enc->attr( $this->formparam( array( 'price', 'idx', 'swpost.lists.type' ) ) ); ?>'.replace('idx', idx)"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input is="flat-pickr" class="form-control listitem-datestart" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'price', 'idx', 'swpost.lists.datestart' ) ) ); ?>'.replace('idx', idx)"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
										v-bind:disabled="item['swpost.lists.siteid'] != siteid"
										v-bind:config="Aimeos.flatpickr.datetime"
										v-model="item['swpost.lists.datestart']" />
								</div>
								<div class="col-sm-12 form-text text-muted help-text">
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input is="flat-pickr" class="form-control listitem-dateend" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'price', 'idx', 'swpost.lists.dateend' ) ) ); ?>'.replace('idx', idx)"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
										v-bind:disabled="item['swpost.lists.siteid'] != siteid"
										v-bind:config="Aimeos.flatpickr.datetime"
										v-model="item['swpost.lists.dateend']" />
								</div>
								<div class="col-sm-12 form-text text-muted help-text">
						...
						<div v-show="item['_ext']" class="col-xl-6 content-block secondary" v-bind:class="{readonly: item['swpost.lists.siteid'] != siteid}">
							<config-table inline-template
								v-bind:index="idx" v-bind:readonly="item['swpost.lists.siteid'] != siteid"
										...
										<tr v-for="(entry, pos) in items" v-bind:key="pos" class="config-item">
											<td class="config-row-key">
												<input is="auto-complete" required class="form-control" v-bind:readonly="readonly" tabindex="<?= $this->get( 'tabindex' ); ?>"
													...
													v-bind:keys="JSON.parse('<?= $enc->attr( $this->config( 'admin/jqadm/swpost/item/price/config/suggest', [] ) ) ?>')"
													v-model="entry.key" />
											</td>
