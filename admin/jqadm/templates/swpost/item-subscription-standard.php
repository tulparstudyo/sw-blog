...
$keys = [
	'Y', 'M', 'W', 'D', 'H',
	'swpost.lists.id', 'swpost.lists.siteid',
	...
	<table class="subscription-list table table-default col-xl-12 content-block"
		...
		<tbody>
			<tr v-for="(entry, idx) in items" v-bind:key="idx"
				v-bind:class="entry['swpost.lists.siteid'] != '<?= $this->site()->siteid() ?>' ? 'readonly' : ''">
				...
				<td class="interval-check">
					<input class="form-control item-id" type="checkbox" tabindex="<?= $this->get( 'tabindex' ); ?>"
						...
						v-bind:checked="entry['swpost.lists.id'] || !entry['attribute.id'] ? 'checked' : ''"
				...
				<td class="actions">
					...
					<input class="item-listid" type="hidden"
						v-bind:name="'<?= $enc->attr( $this->formparam( array( 'subscription', 'idx', 'swpost.lists.id' ) ) ); ?>'.replace( 'idx', idx )"
						v-bind:value="entry['swpost.lists.id']" />

					<input class="item-siteid" type="hidden"
						v-bind:name="'<?= $enc->attr( $this->formparam( array( 'subscription', 'idx', 'swpost.lists.siteid' ) ) ); ?>'.replace( 'idx', idx )"
						v-bind:value="entry['swpost.lists.siteid']" />
