...
/** admin/jqadm/swpost/order/fields
 * List of list and order columns that should be displayed in the swpost order view
 *
 * Changes the list of list and order columns shown by default in the swpost order view.
 * The columns can be changed by the editor as required within the administraiton
 * interface.
 *
 * The names of the colums are in fact the search keys defined by the managers,
 * e.g. "swpost.lists.status" for the status value.
 *
...
$fields = $this->config( 'admin/jqadm/swpost/order/fields', $fields );
		...
		<table class="list-items table table-striped table-hover">
			<thead class="list-header">
				<tr>
					...
					<th v-if="fields.includes('order.base.address.swpostXal')" class="order-base-address-swpostXal">
						<a v-bind:class="sortclass('order.base.address.swpostXal')" v-on:click.prevent="orderby('order.base.address.swpostXal')"
							tabindex="<?= $this->get( 'tabindex', 1 ); ?>" href="#">
					...
					<th class="actions">
						<div class="dropdown filter-columns">
							...
							<ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton-<?= $this->get( 'group' ) ?>">
								...
								<li class="dropdown-item">
									<a v-on:click.prevent.stop="toggleField('order.base.address.swpostXal')" href="#"><label>
										<input v-on:click.capture.stop="toggleField('order.base.address.swpostXal')"
											v-bind:checked="fields.includes('order.base.address.swpostXal')"
											type="checkbox" tabindex="<?= $this->get( 'tabindex', 1 ); ?>" />
										<?= $enc->html( $this->translate( 'admin', 'Zip code' ) ); ?>
									</label></a>
								</li>
			...
			<tbody>
				<tr class="list-search">
					...
					<td v-if="fields.includes('order.base.address.swpostXal')" class="order-base-address-swpostXal">
						<input v-on:change="find($event, 'order.base.address.swpostXal', '=~')" v-bind:value="value('order.base.address.swpostXal')" class="form-control novalidate" tabindex="<?= $this->get( 'tabindex' ); ?>" />
					</td>
				...
				<tr v-for="(item, idx) in items" class="list-item">
					<td v-if="fields.includes('order.base.id')" class="order-base-id">
						<a class="items-field" v-bind:href="'<?= $this->url( $target, $cntl, $action, ['resource' => 'order', 'id' => '_id_'], [], $config ) ?>'.replace('_id_', item.id)">
					...
					<td v-if="fields.includes('order.base.address.company')" class="order-base-address-company">
						<a class="items-field"  v-bind:href="'<?= $this->url( $target, $cntl, $action, ['resource' => 'customer', 'id' => '_id_'], [], $config ) ?>'.replace('_id_', item.attributes['order.base.customerid'])">
					...
					<td v-if="fields.includes('order.base.address.swpostXal')" class="order-base-address-swpostXal">
						<a class="items-field"  v-bind:href="'<?= $this->url( $target, $cntl, $action, ['resource' => 'customer', 'id' => '_id_'], [], $config ) ?>'.replace('_id_', item.attributes['order.base.customerid'])">
							{{ related(item, 'order/base/address', 'order.base.address.swpostXal') }}
						</a>
					</td>
					<td v-if="fields.includes('order.base.address.city')" class="order-base-address-city">
						<a class="items-field"  v-bind:href="'<?= $this->url( $target, $cntl, $action, ['resource' => 'customer', 'id' => '_id_'], [], $config ) ?>'.replace('_id_', item.attributes['order.base.customerid'])">
			...
			<ul class="page-offset pagination">
				...
				--><li class="page-item disabled">
					<button class="page-link" tabindex="<?= $this->get( 'tabindex', 1 ); ?>">
						{{ pagecnt('<?= $enc->html( $this->translate( 'admin', 'Page %1$d of %2$d' ) ); ?>') }}
					</button>
				</li><!--
