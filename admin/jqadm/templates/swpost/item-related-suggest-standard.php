...
<div class="col-xl-6 content-block item-related-suggest">
	...
	<table class="swpost-list table table-default"
		...
		<thead>
			<tr>
				<th>
					<span class="help"><?= $enc->html( $this->translate( 'admin', 'Suggested swposts' ) ); ?></span>
					<div class="form-text text-muted help-text">
						<?= $enc->html( $this->translate( 'admin', 'List of suggested swposts' ) ); ?>
					</div>
				</th>
				<th class="actions">
			...
			<tr v-for="(item, idx) in items" v-bind:key="idx"
				...
				<td v-bind:class="item['css'] || ''">
					<input class="item-listid" type="hidden" v-model="item['swpost.lists.id']"
						v-bind:name="'<?= $enc->attr( $this->formparam( ['related', 'suggest', 'idx', 'swpost.lists.id'] ) ); ?>'.replace( 'idx', idx )" />

					<input class="item-label" type="hidden" v-model="item['swpost.label']"
						v-bind:name="'<?= $enc->attr( $this->formparam( ['related', 'suggest', 'idx', 'swpost.label'] ) ); ?>'.replace( 'idx', idx )" />

					<input class="item-code" type="hidden" v-model="item['swpost.code']"
						v-bind:name="'<?= $enc->attr( $this->formparam( ['related', 'suggest', 'idx', 'swpost.code'] ) ); ?>'.replace( 'idx', idx )" />

					<select is="combo-box" class="form-control custom-select item-refid"
						v-bind:name="'<?= $enc->attr( $this->formparam( ['related', 'suggest', 'idx', 'swpost.lists.refid'] ) ); ?>'.replace( 'idx', idx )"
