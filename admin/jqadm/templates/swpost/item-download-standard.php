...
<div id="download" class="row item-download tab-pane fade" role="tabpanel" aria-labelledby="download">
	<div class="col-lg-6 content-block <?= $this->site()->readonly( $this->get( 'downloadData/swpost.lists.siteid', $this->pageSiteItem->getId() ) ); ?>">
		<div class="form-group row optional">
			<label class="col-sm-4 form-control-label"><?= $enc->html( $this->translate( 'admin', 'File' ) ); ?></label>
			<div class="input-group col-sm-8">
				<input class="item-siteid" type="hidden" name="<?= $enc->attr( $this->formparam( array( 'download', 'swpost.lists.siteid' ) ) ); ?>"
					value="<?= $enc->attr( $this->get( 'downloadData/swpost.lists.siteid' ) ); ?>" />
				<input class="item-listid" type="hidden" name="<?= $enc->attr( $this->formparam( array( 'download', 'swpost.lists.id' ) ) ); ?>"
					value="<?= $enc->attr( $this->get( 'downloadData/swpost.lists.id' ) ); ?>" />
		...
		<div class="form-group row optional">
			...
			<div class="col-sm-8">
				<input class="form-control item-label" type="text" tabindex="<?= $this->get( 'tabindex' ); ?>"
					...
					<?= $this->site()->readonly( $this->get( 'downloadData/swpost.lists.siteid', $this->pageSiteItem->getId() ) ); ?> />
			</div>
