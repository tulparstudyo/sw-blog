...
/** admin/jqadm/post/item/config/suggest
 * List of suggested configuration keys in post item panel
 *
 * Post items can store arbitrary key value pairs. This setting gives editors
 ...
 */
$cfgSuggest = $this->config( 'admin/jqadm/post/item/config/suggest', ['css-class'] );


/** admin/jqadm/post/item/navbar-limit
 * Number of sub-part links in the post sidebar shown by default
 *
 ...
 *
 * @param integer Number of post sub-part links
 * @since 2020.10
 * @category Developer
 * @see admin/jqadm/navbar-limit
 */
$navlimit = $this->config( 'admin/jqadm/post/item/navbar-limit', 7 );
...
/** admin/jqadm/dataset/post
 * Predefined data sets for posts
 *
 * To simplify creating new posts by editors, Aimeos supports the defintion
 * of arbitrary post templates. Each template consists of a name and the
 * associated configuration template for that post type (e.g. a T-Shirt).
 * The template names will be listed in a select box in the post detail view.
 *
 ...
 *        'property' => [['post.property.type' => 'isbn']],
 *      ],
 *      'related' => [
 *        'suggest' => [[], []],
 *      ],
 ...
 * * two select boxes for suggested posts in the "Related" tab
 ...
 * e.g. for prot variants and bundles.
 *
...
<form class="item item-post form-horizontal" method="POST" enctype="multipart/form-data" action="<?= $enc->attr( $this->url( $target, $cntl, $action, $params, [], $config ) ); ?>">
	<input id="item-id" type="hidden" name="<?= $enc->attr( $this->formparam( array( 'item', 'post.id' ) ) ); ?>" value="<?= $enc->attr( $this->get( 'itemData/post.id' ) ); ?>" />
	<input id="item-next" type="hidden" name="<?= $enc->attr( $this->formparam( array( 'next' ) ) ); ?>" value="get" />
	<?= $this->csrf()->formfield(); ?>

	<nav class="main-navbar">
		<h1 class="navbar-brand">
			<span class="navbar-title"><?= $enc->html( $this->translate( 'admin', 'Post' ) ); ?></span>
	...
	<div class="row item-container">
		...
		<div class="col-md-9 item-content tab-content">
			...
			<div id="basic" class="row item-basic tab-pane fade show active" role="tabpanel" aria-labelledby="basic">
				...
				<div class="col-xl-6 content-block vue-block <?= $this->site()->readonly( $this->get( 'itemData/post.siteid' ) ); ?>"
					data-data="<?= $enc->attr( $this->get( 'itemData', new stdClass() ) ) ?>">

					<?php if( $this->config( 'admin/jqadm/dataset/post', [] ) !== [] ) : ?>
						<div class="form-group row optional">
							...
							<div class="col-sm-8">
								<select class="form-control custom-select item-set" tabindex="1"
									name="<?= $enc->attr( $this->formparam( array( 'item', 'post.dataset' ) ) ); ?>"
									<?= $this->site()->readonly( $this->get( 'itemData/post.siteid' ) ); ?> >
									<option value="">
										<?= $enc->html( $this->translate( 'admin', 'None' ) ); ?>
									</option>

									<?php foreach( $this->config( 'admin/jqadm/dataset/post', [] ) as $name => $config ) : ?>
										<option value="<?= $enc->attr( $name ); ?>" <?= $selected( $this->get( 'itemData/post.dataset' ), $name ); ?>
							...
							<div class="col-sm-12 form-text text-muted help-text">
								<?= $enc->html( $this->translate( 'admin', 'Depending on the selected data set, the list of shown fields for the post will be different' ) ); ?>
							</div>
						</div>
					<?php endif ?>
					...
					<?php if( ( $types = $this->get( 'itemTypes', map() )->col( 'post.type.label', 'post.type.code' ) )->count() !== 1 ) : ?>
						<div class="form-group row mandatory">
							<label class="col-sm-4 form-control-label"><?= $enc->html( $this->translate( 'admin', 'Type' ) ); ?></label>
							<div class="col-sm-8">
								<select is="select-component" class="form-control custom-select item-type" required v-bind:tabindex="'1'"
									v-bind:readonly="'<?= $this->site()->readonly( $this->get( 'itemData/post.siteid' ) ); ?>' ? true : false"
									v-bind:name="'<?= $enc->attr( $this->formparam( ['item', 'post.type'] ) ); ?>'"
									v-bind:text="'<?= $enc->html( $this->translate( 'admin', 'Please select' ) ); ?>'"
									v-bind:items="JSON.parse('<?= $enc->attr( $types->toArray() ) ?>')"
									v-model="data['post.type']" >
									<option value="<?= $enc->attr( $this->get( 'itemData/post.type' ) ) ?>">
										<?= $enc->html( $types[$this->get( 'itemData/post.type', '' )] ?? $this->translate( 'admin', 'Please select' ) ) ?>
									</option>
								</select>
							</div>
						</div>
					<?php else : ?>
						<input class="item-type" type="hidden"
							name="<?= $enc->attr( $this->formparam( array( 'item', 'post.type' ) ) ); ?>"
					...
					<div class="form-group row mandatory">
						...
						<div class="col-sm-8">
							<input class="form-control item-code" type="text" required="required" tabindex="1"
								name="<?= $enc->attr( $this->formparam( array( 'item', 'post.code' ) ) ); ?>"
								placeholder="<?= $enc->attr( $this->translate( 'admin', 'EAN, SKU or article number (required)' ) ); ?>"
								value="<?= $enc->attr( $this->get( 'itemData/post.code' ) ); ?>"
								<?= $this->site()->readonly( $this->get( 'itemData/post.siteid' ) ); ?> />
						</div>
						<div class="col-sm-12 form-text text-muted help-text">
					...
					<div class="form-group row mandatory">
						...
						<div class="col-sm-12 form-text text-muted help-text">
							<?= $enc->html( $this->translate( 'admin', 'Internal article name, will be used on the web site and for searching only if no other post names in any language exist' ) ); ?>
						</div>
					</div>
					...
					<div class="form-group row optional advanced">
						...
						<div class="col-sm-8">
							<input class="form-control item-label" type="text" tabindex="1"
								name="<?= $this->formparam( array( 'item', 'post.url' ) ); ?>"
								placeholder="<?= $enc->attr( $this->translate( 'admin', 'Name in URL (optional)' ) ); ?>"
								value="<?= $enc->attr( $this->get( 'itemData/post.url' ) ); ?>"
								<?= $this->site()->readonly( $this->get( 'itemData/post.siteid' ) ); ?> />
						</div>
						<div class="col-sm-12 form-text text-muted help-text">
							<?= $enc->html( $this->translate( 'admin', 'The name of the post shown in the URL, will be used if no language specific URL segment exists' ) ); ?>
						</div>
					</div>
					<div class="form-group row optional advanced">
						...
						<div class="col-sm-8">
							<input class="form-control item-scale" type="number" tabindex="1" min="0.001" step="0.001"
								name="<?= $enc->attr( $this->formparam( array( 'item', 'post.scale' ) ) ); ?>"
								value="<?= $enc->attr( $this->datetime( $this->get( 'itemData/post.scale', 1 ) ) ); ?>"
								<?= $this->site()->readonly( $this->get( 'itemData/post.siteid' ) ); ?> />
						</div>
						<div class="col-sm-12 form-text text-muted help-text">
					...
					<div class="form-group row optional advanced">
						...
						<div class="col-sm-8">
							<input is="flat-pickr" class="form-control item-datestart" type="datetime-local" tabindex="1"
								name="<?= $enc->attr( $this->formparam( array( 'item', 'post.datestart' ) ) ); ?>"
								placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
								v-bind:value="'<?= $enc->attr( $this->datetime( $this->get( 'itemData/post.datestart' ) ) ); ?>'"
								v-bind:disabled="'<?= $this->site()->readonly( $this->get( 'itemData/post.siteid' ) ); ?>' !== ''"
					...
					<div class="form-group row optional advanced">
						...
						<div class="col-sm-8">
							<input is="flat-pickr" class="form-control item-dateend" type="datetime-local" tabindex="1"
								name="<?= $enc->attr( $this->formparam( array( 'item', 'post.dateend' ) ) ); ?>"
								placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
								v-bind:value="'<?= $enc->attr( $this->datetime( $this->get( 'itemData/post.dateend' ) ) ); ?>'"
								v-bind:disabled="'<?= $this->site()->readonly( $this->get( 'itemData/post.siteid' ) ); ?>' !== ''"
								v-bind:config="Aimeos.flatpickr.datetime" />
						</div>
						<div class="col-sm-12 form-text text-muted help-text">
					...
					<div class="form-group row optional advanced">
						...
						<div class="col-sm-8">
							<input is="flat-pickr" class="form-control item-ctime" type="datetime-local" tabindex="1"
								name="<?= $enc->attr( $this->formparam( array( 'item', 'post.ctime' ) ) ); ?>"
								placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
								v-bind:value="'<?= $enc->attr( $this->datetime( $this->get( 'itemData/post.ctime' ) ) ); ?>'"
								v-bind:disabled="'<?= $this->site()->readonly( $this->get( 'itemData/post.siteid' ) ); ?>' !== ''"
								v-bind:config="Aimeos.flatpickr.datetime" />
						</div>
						<div class="col-sm-12 form-text text-muted help-text">
							<?= $enc->html( $this->translate( 'admin', 'Since when the post is available, used for sorting in the front-end' ) ); ?>
						</div>
					</div>
					<div class="form-group row optional advanced warning">
						...
						<div class="col-sm-8">
							<input class="form-control item-target" type="text" tabindex="1"
								name="<?= $enc->attr( $this->formparam( array( 'item', 'post.target' ) ) ); ?>"
								placeholder="<?= $enc->attr( $this->translate( 'admin', 'Route or page ID (optional)' ) ); ?>"
								value="<?= $enc->attr( $this->get( 'itemData/post.target' ) ); ?>"
								<?= $this->site()->readonly( $this->get( 'itemData/post.siteid' ) ); ?> />
						</div>
						<div class="col-sm-12 form-text text-muted help-text">
							<?= $enc->html( $this->translate( 'admin', 'Route name or page ID of the post detail page if this post should shown on a different page' ) ); ?>
						</div>
					</div>
				</div><!--
				...
				--><div class="col-xl-6 content-block vue-block <?= $this->site()->readonly( $this->get( 'itemData/post.siteid' ) ); ?>"
					...
					<config-table inline-template
						v-bind:readonly="data['post.siteid'] != '<?= $this->site()->siteid() ?>'"
								...
								<tr v-for="(entry, pos) in items" v-bind:key="pos" class="config-item">
									<td class="config-row-key">
										<input is="auto-complete" required class="form-control" v-bind:readonly="readonly" tabindex="1"
											...
											v-bind:keys="JSON.parse('<?= $enc->attr( $this->config( 'admin/jqadm/post/item/config/suggest', ['css-class'] ) ) ?>')"
											v-model="entry.key" />
									</td>
