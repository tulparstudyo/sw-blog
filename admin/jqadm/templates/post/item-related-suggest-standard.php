...
<div class="col-xl-6 content-block item-related-suggest">
	...
	<table class="post-list table table-default"
		...
		<thead>
			<tr>
				<th>
					<span class="help"><?= $enc->html( $this->translate( 'admin', 'Suggested posts' ) ); ?></span>
					<div class="form-text text-muted help-text">
						<?= $enc->html( $this->translate( 'admin', 'List of suggested posts' ) ); ?>
					</div>
				</th>
				<th class="actions">
			...
			<tr v-for="(item, idx) in items" v-bind:key="idx"
				...
				<td v-bind:class="item['css'] || ''">
					<input class="item-listid" type="hidden" v-model="item['post.lists.id']"
						v-bind:name="'<?= $enc->attr( $this->formparam( ['related', 'suggest', 'idx', 'post.lists.id'] ) ); ?>'.replace( 'idx', idx )" />

					<input class="item-label" type="hidden" v-model="item['post.label']"
						v-bind:name="'<?= $enc->attr( $this->formparam( ['related', 'suggest', 'idx', 'post.label'] ) ); ?>'.replace( 'idx', idx )" />

					<input class="item-code" type="hidden" v-model="item['post.code']"
						v-bind:name="'<?= $enc->attr( $this->formparam( ['related', 'suggest', 'idx', 'post.code'] ) ); ?>'.replace( 'idx', idx )" />

					<select is="combo-box" class="form-control custom-select item-refid"
						v-bind:name="'<?= $enc->attr( $this->formparam( ['related', 'suggest', 'idx', 'post.lists.refid'] ) ); ?>'.replace( 'idx', idx )"
