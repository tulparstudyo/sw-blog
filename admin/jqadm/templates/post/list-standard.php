 ...
 * @category Developer
 */
$default = $this->config( 'admin/jqadm/post/fields', ['image', 'post.id', 'post.status', 'post.type', 'post.code', 'post.label'] );
...
$columnList = [
	...
	'post.type' => $this->translate( 'admin', 'Type' ),
	'post.code' => $this->translate( 'admin', 'Code' ),
	'post.label' => $this->translate( 'admin', 'Label' ),
	'post.datestart' => $this->translate( 'admin', 'Start date' ),
	'post.dateend' => $this->translate( 'admin', 'End date' ),
	'post.config' => $this->translate( 'admin', 'Config' ),
...
<nav class="main-navbar">
	...
	<?= $this->partial(
		$this->config( 'admin/jqadm/partial/navsearch', 'common/partials/navsearch-standard' ), [
			'filter' => $this->session( 'aimeos/admin/jqadm/post/filter', [] ),
...
<form class="list list-post" method="POST" action="<?= $enc->attr( $this->url( $target, $controller, $action, $searchParams, [], $config ) ); ?>">
	<?= $this->csrf()->formfield(); ?>
		...
		<thead class="list-header">
			<tr>
				...
				<?= $this->partial(
						...
						'sort' => $this->session( 'aimeos/admin/jqadm/post/sort', 'post.id' )]
					);
				?>
			...
			<?= $this->partial(
				$this->config( 'admin/jqadm/partial/listsearch', 'common/partials/listsearch-standard' ), [
					...
					'data' => [
						...
						'post.type' => ['op' => '==', 'type' => 'select', 'val' => $typeList],
						'post.code' => [],
						'post.label' => [],
						'post.datestart' => ['op' => '-', 'type' => 'datetime-local'],
						'post.dateend' => ['op' => '-', 'type' => 'datetime-local'],
						'post.config' => ['op' => '~='],
			...
			<?php foreach( $this->get( 'items', [] ) as $id => $item ) : ?>
				...
				<tr class="list-item <?= $this->site()->readonly( $item->getSiteId() ); ?>" data-label="<?= $enc->attr( $item->getLabel() ) ?>">
					...
					<?php if( in_array( 'post.id', $fields ) ) : ?>
						<td class="post-id"><a class="items-field" href="<?= $url; ?>" tabindex="1"><?= $enc->html( $item->getId() ); ?></a></td>
					<?php endif; ?>
					...
					<?php endif; ?>
					<?php if( in_array( 'post.type', $fields ) ) : ?>
						<td class="post-type"><a class="items-field" href="<?= $url; ?>"><?= $enc->html( $item->getType() ); ?></a></td>
					<?php endif; ?>
					<?php if( in_array( 'post.code', $fields ) ) : ?>
						<td class="post-code"><a class="items-field" href="<?= $url; ?>"><?= $enc->html( $item->getCode() ); ?></a></td>
					<?php endif; ?>
					<?php if( in_array( 'post.label', $fields ) ) : ?>
						<td class="post-label"><a class="items-field" href="<?= $url; ?>"><?= $enc->html( $item->getLabel() ); ?></a></td>
					<?php endif; ?>
					<?php if( in_array( 'post.datestart', $fields ) ) : ?>
						<td class="post-datestart"><a class="items-field" href="<?= $url; ?>"><?= $enc->html( $item->getDateStart() ); ?></a></td>
					<?php endif; ?>
					<?php if( in_array( 'post.dateend', $fields ) ) : ?>
						<td class="post-dateend"><a class="items-field" href="<?= $url; ?>"><?= $enc->html( $item->getDateEnd() ); ?></a></td>
					<?php endif; ?>
					<?php if( in_array( 'post.config', $fields ) ) : ?>
						<td class="post-config config-item">
