		...
		<thead>
			<tr>
				...
				<th>
					...
					<div class="form-text text-muted help-text">
						<?= $enc->html( $this->translate( 'admin', 'Variant post attributes that are stored with the ordered posts' ) ); ?>
					</div>
				</th>
				<th class="actions">
			...
			<tr v-for="(item, idx) in items" v-bind:key="idx"
				...
				<td v-bind:class="item['css'] || ''">
					<input class="item-listid" type="hidden" v-model="item['post.lists.id']"
						v-bind:name="'<?= $enc->attr( $this->formparam( ['characteristic', 'variant', 'idx', 'post.lists.id'] ) ); ?>'.replace( 'idx', idx )" />
					...
					<select is="combo-box" class="form-control custom-select item-refid"
						v-bind:name="'<?= $enc->attr( $this->formparam( ['characteristic', 'variant', 'idx', 'post.lists.refid'] ) ); ?>'.replace( 'idx', idx )"
						v-bind:readonly="checkSite('post.lists.siteid', idx) || item['post.lists.id'] != ''"
