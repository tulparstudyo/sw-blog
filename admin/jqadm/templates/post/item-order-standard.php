...
/** admin/jqadm/post/order/fields
 * List of list and order columns that should be displayed in the post order view
 *
 * Changes the list of list and order columns shown by default in the post order view.
 * The columns can be changed by the editor as required within the administraiton
 * interface.
 *
 * The names of the colums are in fact the search keys defined by the managers,
 * e.g. "post.lists.status" for the status value.
 *
...
$fields = $this->config( 'admin/jqadm/post/order/fields', $fields );
		...
		<table class="list-items table table-striped table-hover">
			<thead class="list-header">
				<tr>
					...
					<th v-if="fields.includes('order.base.address.postXal')" class="order-base-address-postXal">
						<a v-bind:class="sortclass('order.base.address.postXal')" v-on:click.prevent="orderby('order.base.address.postXal')"
							tabindex="<?= $this->get( 'tabindex', 1 ); ?>" href="#">
					...
					<th class="actions">
						<div class="dropdown filter-columns">
							...
							<ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton-<?= $this->get( 'group' ) ?>">
								...
								<li class="dropdown-item">
									<a v-on:click.prevent.stop="toggleField('order.base.address.postXal')" href="#"><label>
										<input v-on:click.capture.stop="toggleField('order.base.address.postXal')"
											v-bind:checked="fields.includes('order.base.address.postXal')"
											type="checkbox" tabindex="<?= $this->get( 'tabindex', 1 ); ?>" />
										<?= $enc->html( $this->translate( 'admin', 'Zip code' ) ); ?>
									</label></a>
								</li>
			...
			<tbody>
				<tr class="list-search">
					...
					<td v-if="fields.includes('order.base.address.postXal')" class="order-base-address-postXal">
						<input v-on:change="find($event, 'order.base.address.postXal', '=~')" v-bind:value="value('order.base.address.postXal')" class="form-control novalidate" tabindex="<?= $this->get( 'tabindex' ); ?>" />
					</td>
				...
				<tr v-for="(item, idx) in items" class="list-item">
					...
					<td v-if="fields.includes('order.base.address.postXal')" class="order-base-address-postXal">
						<a class="items-field"  v-bind:href="'<?= $this->url( $target, $cntl, $action, ['resource' => 'customer', 'id' => '_id_'], [], $config ) ?>'.replace('_id_', item.attributes['order.base.customerid'])">
							{{ related(item, 'order/base/address', 'order.base.address.postXal') }}
						</a>
					</td>
					<td v-if="fields.includes('order.base.address.city')" class="order-base-address-city">
						<a class="items-field"  v-bind:href="'<?= $this->url( $target, $cntl, $action, ['resource' => 'customer', 'id' => '_id_'], [], $config ) ?>'.replace('_id_', item.attributes['order.base.customerid'])">
