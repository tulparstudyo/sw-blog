...
$keys = ['post.lists.siteid', 'post.lists.id', 'post.lists.refid', 'post.label', 'post.code', 'post.status', 'stock.id', 'stock.stocklevel'];
				...
				<div v-for="(item, idx) in items" v-bind:key="idx" class="group-item card">
					...
					<div v-bind:id="'item-selection-group-item-' + idx" v-bind:class="getCss(idx)"
						...
						<div class="card-tools-right">
							<a v-if="item['post.id']" class="btn btn-card-header act-view fa" target="_blank" tabindex="<?= $this->get( 'tabindex' ); ?>"
								v-bind:href="'<?= $enc->attr( $this->url( $target, $cntl, $action, ['id' => '_ID_'] + $this->get( 'pageParams', [] ), [], $config ) ); ?>'.replace('_ID_', item['post.id'])"
					...
					<div v-bind:id="'item-selection-group-data-' + idx" v-bind:class="getCss(idx)"
						...
						<input type="hidden" v-model="item['post.id']"
							v-bind:name="'<?= $enc->attr( $this->formparam( ['selection', 'idx', 'post.id'] ) ); ?>'.replace('idx', idx)" />
						<input type="hidden" v-model="item['post.lists.id']"
							v-bind:name="'<?= $enc->attr( $this->formparam( ['selection', 'idx', 'post.lists.id'] ) ); ?>'.replace('idx', idx)" />
							...
							<div class="form-group row mandatory">
								...
								<div class="col-sm-8">
									<select class="form-control custom-select item-status" required="required" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'selection', 'idx', 'post.status' ) ) ); ?>'.replace('idx', idx)"
										v-bind:readonly="checkSite('post.siteid', idx)"
										v-model="item['post.status']" >
										<option value="1" v-bind:selected="item['post.status'] == 1" >
											<?= $enc->html( $this->translate( 'mshop/code', 'status:1' ) ); ?>
										</option>
										<option value="0" v-bind:selected="item['post.status'] == 0" >
											<?= $enc->html( $this->translate( 'mshop/code', 'status:0' ) ); ?>
										</option>
										<option value="-1" v-bind:selected="item['post.status'] == -1" >
											<?= $enc->html( $this->translate( 'mshop/code', 'status:-1' ) ); ?>
										</option>
										<option value="-2" v-bind:selected="item['post.status'] == -2" >
							...
							<div class="form-group row mandatory">
								...
								<div class="col-lg-8">
									<input is="auto-complete"
										v-model="item['post.code']"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'EAN, SKU or article number (required)' ) ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'selection', 'idx', 'post.code' ) ) ); ?>'.replace('idx', idx)"
										v-bind:readonly="checkSite('post.siteid', idx) || item['post.lists.id'] != ''"
										v-bind:tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:keys="getArticles"
										v-bind:required="'required'"
										v-on:input="updatePostItem(idx, ...arguments)" />
								</div>
							...
							<div class="form-group row mandatory">
								...
								<div class="col-lg-8">
									<input class="form-control item-label" type="text" required="required" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'selection', 'idx', 'post.label' ) ) ); ?>'.replace('idx', idx)"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'Internal name (required)' ) ); ?>"
										v-bind:readonly="checkSite('post.siteid', idx)"
										v-model="item['post.label']" />
								</div>
								<div class="col-sm-12 form-text text-muted help-text">
									<?= $enc->html( $this->translate( 'admin', 'Internal article name, will be used on the web site if no post name for the language is available' ) ); ?>
								</div>
							</div>
							...
							<table class="selection-item-attributes table table-default">
								...
								<tbody is="draggable" handle=".act-move" tag="tbody">
									...
									<tr v-for="(attr, attridx) in (item['attr'] || [])">
										<td>
											<input class="item-attr-listid" type="hidden" v-model="attr['post.lists.id']"
												v-bind:name="'<?= $enc->attr( $this->formparam( ['selection', 'idx', 'attr', 'attridx', 'post.lists.id'] ) ); ?>'.replace('idx', idx).replace('attridx', attridx)" />

											<input class="item-attr-siteid" type="hidden" v-model="attr['post.lists.siteid']"
												v-bind:name="'<?= $enc->attr( $this->formparam( ['selection', 'idx', 'attr', 'attridx', 'post.lists.siteid'] ) ); ?>'.replace('idx', idx).replace('attridx', attridx)" />
											...
											<select is="combo-box" class="form-control custom-select item-attr-refid"
												v-bind:name="'<?= $enc->attr( $this->formparam( ['selection', 'idx', 'attr', 'attridx', 'post.lists.refid'] ) ); ?>'.replace('idx', idx).replace('attridx', attridx)"
												v-bind:readonly="checkSite('post.lists.siteid', idx, attridx) || attr['post.lists.id'] != ''"
												v-bind:tabindex="'<?= $this->get( 'tabindex' ); ?>'"
												...
												v-model="attr['post.lists.refid']" >
											</select>
										</td>
										<td class="actions">
											<div v-if="!checkSite('post.lists.siteid', idx, attridx)"
