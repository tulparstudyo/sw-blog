...
<div class="col-xl-12 content-block vue-block" data-key="characteristic/property"
	...
	<property-table inline-template ref="characteristic/property"
		v-bind:domain="'post'" v-bind:siteid="'<?= $this->site()->siteid() ?>'"
		v-bind:types="JSON.parse('<?= $enc->attr( $this->get( 'propertyTypes', map() )->col( 'post.property.type.label', 'post.property.type.code' )->toJson( JSON_FORCE_OBJECT ) ) ?>')"
				...
				<tr>
					<th colspan="3">
						...
						<div class="form-text text-muted help-text">
							<?= $enc->html( $this->translate( 'admin', 'Post characteristics that are not shared with other posts' ) ); ?>
						</div>
					</th>
					<th class="actions">
						<a class="btn act-list fa" tabindex="<?= $this->get( 'tabindex' ); ?>" target="_blank"
							...
							href="<?= $enc->attr( $this->url( $starget, $scntl, $saction, ['resource' => 'type/post/property'] + $this->get( 'pageParams', [] ), [], $sconfig ) ); ?>">
						</a>
			...
			<tbody>
				<tr v-for="(propdata, propidx) in items" v-bind:key="propidx" v-bind:class="{readonly: readonly(propidx)}">
					<td class="property-type">
						<input class="item-id" type="hidden" v-model="propdata['post.property.id']"
							v-bind:name="'<?= $enc->attr( $this->formparam( ['characteristic', 'property', '_idx_', 'post.property.id'] ) ); ?>'.replace('_idx_', propidx)" />

						<select is="select-component" required class="form-control custom-select item-type" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ); ?>"
							v-bind:name="'<?= $enc->attr( $this->formparam( ['characteristic', 'property', '_idx_', 'post.property.type'] ) ); ?>'.replace('_idx_', propidx)"
							v-bind:text="'<?= $enc->html( $this->translate( 'admin', 'Please select' ) ); ?>'"
							v-bind:readonly="readonly(propidx)"
							v-bind:items="types"
							v-model="propdata['post.property.type']" >
						</select>
					</td>
					<td class="property-language">
						<select is="select-component" class="form-control custom-select item-languageid" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ); ?>"
							v-bind:name="'<?= $enc->attr( $this->formparam( ['characteristic', 'property', '_idx_', 'post.property.languageid'] ) ); ?>'.replace('_idx_', propidx)"
							v-bind:all="'<?= $enc->html( $this->translate( 'admin', 'All' ) ); ?>'"
							v-bind:readonly="readonly(propidx)"
							v-bind:items="languages"
							v-model="propdata['post.property.languageid']" >
						</select>
					</td>
					<td class="property-value">
						<input class="form-control item-value" type="text" required="required" tabindex="<?= $this->get( 'tabindex' ); ?>"
							v-bind:name="'<?= $enc->attr( $this->formparam( ['characteristic', 'property', '_idx_', 'post.property.value'] ) ); ?>'.replace('_idx_', propidx)"
							placeholder="<?= $enc->attr( $this->translate( 'admin', 'Property value (required)' ) ); ?>"
							v-bind:readonly="readonly(propidx)"
							v-model="propdata['post.property.value']" >
					</td>
					<td class="actions">
