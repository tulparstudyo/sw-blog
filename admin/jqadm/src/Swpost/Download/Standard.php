...
namespace Aimeos\Admin\JQAdm\Swpost\Download;

sprintf( 'download' ); // for translation


/**
 * Default implementation of swpost download JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/swpost/download/name
	 * Name of the download subpart used by the JQAdm swpost implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Swpost\Download\Myname".
	...
	{
		$view = $this->getObject()->addData( $this->getView() );
		$data['swpost.lists.siteid'] = $this->getContext()->getLocale()->getSiteId();
	...
	{
		/** admin/jqadm/swpost/download/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/swpost/download/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/swpost/download/decorators/global
		 * @see admin/jqadm/swpost/download/decorators/local
		 */

		/** admin/jqadm/swpost/download/decorators/global
		 ...
		 *
		 *  admin/jqadm/swpost/download/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/swpost/download/decorators/excludes
		 * @see admin/jqadm/swpost/download/decorators/local
		 */

		/** admin/jqadm/swpost/download/decorators/local
		 ...
		 *
		 *  admin/jqadm/swpost/download/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/swpost/download/decorators/global
		 */
		return $this->createSubClient( 'swpost/download/' . $type, $name );
	}
	...
	{
		/** admin/jqadm/swpost/download/standard/subparts
		 * List of JQAdm sub-clients rendered within the swpost download section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/swpost/download/standard/subparts', [] );
	}
	...
	protected function fromArray( \Aimeos\MShop\Swpost\Item\Iface $item, array $data ) : \Aimeos\MShop\Swpost\Item\Iface
	{
		...
		$prodManager = \Aimeos\MShop::create( $context, 'swpost' );
		...
		{
			$listId = $this->getValue( $data, 'swpost.lists.id' );
	...
	protected function toArray( \Aimeos\MShop\Swpost\Item\Iface $item, bool $copy = false ) : array
	{
		...
		$data = ['swpost.lists.siteid' => $siteId];
		...
		foreach( $item->getListItems( 'attribute', 'hidden', 'download', false ) as $listItem )
		{
			...
			catch( \Exception $e ) { ; } // Show swpost even if file isn't available any more
		}
	...
	{
		/** admin/jqadm/swpost/download/template-item
		 * Relative path to the HTML body template of the download subpart for swposts.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/swpost/download/template-item';
		$default = 'swpost/item-download-standard';
