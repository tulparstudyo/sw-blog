...
namespace Aimeos\Admin\JQAdm\Swpost\Subscription;

sprintf( 'subscription' ); // for translation


/**
 * Default implementation of swpost subscription JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/swpost/subscription/name
	 * Name of the subscription subpart used by the JQAdm swpost implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Swpost\Subscription\Myname".
	...
	{
		/** admin/jqadm/swpost/subscription/decorators/excludes
		 * Excludes decorators added by the "common" subscription from the swpost JQAdm client
		 *
		 ...
		 *
		 *  admin/jqadm/swpost/subscription/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/swpost/subscription/decorators/global
		 * @see admin/jqadm/swpost/subscription/decorators/local
		 */

		/** admin/jqadm/swpost/subscription/decorators/global
		 ...
		 *
		 *  admin/jqadm/swpost/subscription/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/swpost/subscription/decorators/excludes
		 * @see admin/jqadm/swpost/subscription/decorators/local
		 */

		/** admin/jqadm/swpost/subscription/decorators/local
		 ...
		 *
		 *  admin/jqadm/swpost/subscription/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/swpost/subscription/decorators/global
		 */
		return $this->createSubClient( 'swpost/subscription/' . $type, $name );
	}
	...
	protected function getIntervalItems() : \Aimeos\Map
	{
		...
		$expr = [
			...
			$search->compare( '==', 'attribute.domain', 'swpost' ),
		];
	...
	{
		/** admin/jqadm/swpost/subscription/standard/subparts
		 * List of JQAdm sub-clients rendered within the swpost subscription section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/swpost/subscription/standard/subparts', [] );
	}
	...
	protected function toArray( \Aimeos\MShop\Swpost\Item\Iface $item, bool $copy = false ) : array
	{
		...
		foreach( $this->getIntervalItems() as $attrId => $attrItem )
		{
			...
			{
				$list['swpost.lists.siteid'] = (string) $map[$attrId]->getSiteId();
				$list['swpost.lists.id'] = (string) $map[$attrId]->getId();
			}
			else
			{
	...
	{
		/** admin/jqadm/swpost/subscription/template-item
		 * Relative path to the HTML body template of the subscription subpart for swposts.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/swpost/subscription/template-item';
		$default = 'swpost/item-subscription-standard';
