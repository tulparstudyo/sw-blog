...
namespace Aimeos\Admin\JQAdm\Swpost\Text;

sprintf( 'text' ); // for translation


/**
 * Default implementation of swpost text JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/swpost/text/name
	 * Name of the text subpart used by the JQAdm swpost implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Swpost\Text\Myname".
	...
	public function addData( \Aimeos\MW\View\Iface $view ) : \Aimeos\MW\View\Iface
	{
		...
		$search->setConditions( $search->compare( '==', 'text.type.domain', 'swpost' ) );
		$search->setSortations( array( $search->sort( '+', 'text.type.position' ) ) );

		$listSearch = $listTypeManager->createSearch( true )->setSlice( 0, 10000 );
		$listSearch->setConditions( $listSearch->compare( '==', 'swpost.lists.type.domain', 'text' ) );
	...
	public function save() : ?string
	{
		$view = $this->getView();
        $data = [];
        $texts = $view->param( 'text', [] );
        foreach($texts as $text){
            $data[] = $text;
        }
		$view->item = $this->fromArray( $view->item, $data  );
	...
	{
		/** admin/jqadm/popost/text/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/swpost/text/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/swpost/text/decorators/global
		 * @see admin/jqadm/swpost/text/decorators/local
		 */

		/** admin/jqadm/swpost/text/decorators/global
		 ...
		 *
		 *  admin/jqadm/swpost/text/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/swpost/text/decorators/excludes
		 * @see admin/jqadm/swpost/text/decorators/local
		 */

		/** admin/jqadm/swpost/text/decorators/local
		 ...
		 *
		 *  admin/jqadm/swpost/text/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/swpost/text/decorators/global
		 */
		return $this->createSubClient( 'swpost/text/' . $type, $name );
	}
	...
	{
		/** admin/jqadm/swpost/text/standard/subparts
		 * List of JQAdm sub-clients rendered within the swpost text section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/swpost/text/standard/subparts', [] );
	}
	...
	{
		/** admin/jqadm/swpost/text/template-item
		 * Relative path to the HTML body template of the text subpart for swposts.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/swpost/text/template-item';
		$default = 'swpost/item-text-standard';
