...
namespace Aimeos\Admin\JQAdm\Swpost\Supplier;

sprintf( 'supplier' ); // for translation


/**
 * Default implementation of swpost supplier JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/swpost/supplier/name
	 * Name of the supplier subpart used by the JQAdm swpost implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Swpost\Supplier\Myname".
	...
	public function delete() : ?string
	{
		...
		$expr = array(
			...
			$search->compare( '==', 'supplier.lists.domain', 'swpost' )
		);
	...
	{
		/** admin/jqadm/swpost/supplier/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/swpost/supplier/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/swpost/supplier/decorators/global
		 * @see admin/jqadm/swpost/supplier/decorators/local
		 */

		/** admin/jqadm/swpost/supplier/decorators/global
		 ...
		 *
		 *  admin/jqadm/swpost/supplier/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @supplier Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/swpost/supplier/decorators/excludes
		 * @see admin/jqadm/swpost/supplier/decorators/local
		 */

		/** admin/jqadm/swpost/supplier/decorators/local
		 ...
		 *
		 *  admin/jqadm/swpost/supplier/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/swpost/supplier/decorators/global
		 */
		return $this->createSubClient( 'swpost/supplier/' . $type, $name );
	}
	...
	{
		/** admin/jqadm/swpost/supplier/standard/subparts
		 * List of JQAdm sub-clients rendered within the swpost supplier section
		 *
		 ...
		 * @supplier Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/swpost/supplier/standard/subparts', [] );
	}
	...
	/**
	 * Returns the supplier list items for the given swpost ID
	 *
	 * @param string $prodid Unique swpost ID
	 ...
	 */
	protected function fromArray( \Aimeos\MShop\Swpost\Item\Iface $item, array $data )
	{
		$manager = \Aimeos\MShop::create( $this->getContext(), 'supplier/lists' );
		...
		foreach( $data as $idx => $entry )
		{
			...
			$list[] = $litem->setParentId( $this->getValue( $entry, 'supplier.id' ) )->setDomain( 'swpost' )
	...
	{
		/** admin/jqadm/swpost/supplier/template-item
		 * Relative path to the HTML body template of the supplier subpart for swposts.
		 *
		 ...
		 * @supplier Developer
		 */
		$tplconf = 'admin/jqadm/swpost/supplier/template-item';
		$default = 'swpost/item-supplier-standard';
