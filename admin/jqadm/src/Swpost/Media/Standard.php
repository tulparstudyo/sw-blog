...
namespace Aimeos\Admin\JQAdm\Swpost\Media;

sprintf( 'media' ); // for translation


/**
 * Default implementation of swpost media JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/swpost/media/name
	 * Name of the media subpart used by the JQAdm swpost implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Swpost\Media\Myname".
	...
	/**
	 * Adds the required data used in the swpost template
	 *
	...
	public function addData( \Aimeos\MW\View\Iface $view ) : \Aimeos\MW\View\Iface
	{
		...
		$listTypeManager = \Aimeos\MShop::create( $context, 'swpost/lists/type' );

		$search = $typeManager->createSearch( true )->setSlice( 0, 10000 );
		$search->setConditions( $search->compare( '==', 'media.type.domain', 'swpost' ) );
		$search->setSortations( [$search->sort( '+', 'media.type.position' )] );

		$listSearch = $listTypeManager->createSearch( true )->setSlice( 0, 10000 );
		$listSearch->setConditions( $listSearch->compare( '==', 'swpost.lists.type.domain', 'media' ) );
		$listSearch->setSortations( [$listSearch->sort( '+', 'swpost.lists.type.position' )] );
	...
	{
		/** admin/jqadm/swpost/media/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/swpost/media/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/swpost/media/decorators/global
		 * @see admin/jqadm/swpost/media/decorators/local
		 */

		/** admin/jqadm/swpost/media/decorators/global
		 ...
		 *
		 *  admin/jqadm/swpost/media/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/swpost/media/decorators/excludes
		 * @see admin/jqadm/swpost/media/decorators/local
		 */

		/** admin/jqadm/swpost/media/decorators/local
		 ...
		 *
		 *  admin/jqadm/swpost/media/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/swpost/media/decorators/global
		 */
		return $this->createSubClient( 'swpost/media/' . $type, $name );
	}


	/**
	 * Adds the swpost variant attributes to the media item
	 * Then, the images will only be shown if the customer selected the swpost variant
	 *
	 * @param \Aimeos\MShop\Media\Item\Iface $mediaItem Media item, maybe with referenced attribute items
	 * @param \Aimeos\MShop\Common\Item\Lists\Iface[] $attrListItems Swpost list items referencing variant attributes
	 ...
	 *
	 * @param \Aimeos\MShop\Swpost\Item\Iface $item Swpost item including media reference
	 * @param \Aimeos\Map $listItems Media list items to be removed
	 * @return \Aimeos\MShop\Swpost\Item\Iface Modified swpost item
	 */
	protected function deleteMediaItems( \Aimeos\MShop\Swpost\Item\Iface $item, \Aimeos\Map $listItems ) : \Aimeos\MShop\Swpost\Item\Iface
	{
		$context = $this->getContext();
		...
		{
			$func = $search->createFunction( 'swpost:has', ['media', $listItem->getType(), $listItem->getRefId()] );
	...
	{
		/** admin/jqadm/swpost/media/standard/subparts
		 * List of JQAdm sub-clients rendered within the swpost media section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/swpost/media/standard/subparts', [] );
	}
	...
	protected function fromArray( \Aimeos\MShop\Swpost\Item\Iface $item, array $data ) : \Aimeos\MShop\Swpost\Item\Iface
	{
		...
		foreach( $data as $idx => $entry )
		{
			$listType = $entry['swpost.lists.type'];
	...
	protected function toArray( \Aimeos\MShop\Swpost\Item\Iface $item, bool $copy = false ) : array
	{
		...
		foreach( $item->getListItems( 'media', null, null, false ) as $listItem )
		{
			...
			$list['swpost.lists.datestart'] = str_replace( ' ', 'T', $list['swpost.lists.datestart'] );
			$list['swpost.lists.dateend'] = str_replace( ' ', 'T', $list['swpost.lists.dateend'] );
			$list['config'] = [];
	...
	{
		/** admin/jqadm/swpost/media/template-item
		 * Relative path to the HTML body template of the media subpart for swposts.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/swpost/media/template-item';
		$default = 'swpost/item-media-standard';
