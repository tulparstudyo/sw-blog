...
namespace Aimeos\Admin\JQAdm\Swpost\Characteristic\Property;


/**
 * Default implementation of swpost property JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/swpost/characteristic/property/name
	 * Name of the characteristic/property subpart used by the JQAdm swpost implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Swpost\Characteristic\Property\Myname".
	...
	{
		/** admin/jqadm/swpost/characteristic/property/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/swpost/characteristic/property/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/swpost/characteristic/property/decorators/global
		 * @see admin/jqadm/swpost/characteristic/property/decorators/local
		 */

		/** admin/jqadm/swpost/characteristic/property/decorators/global
		 ...
		 *
		 *  admin/jqadm/swpost/characteristic/property/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/swpost/characteristic/property/decorators/excludes
		 * @see admin/jqadm/swpost/characteristic/property/decorators/local
		 */

		/** admin/jqadm/swpost/characteristic/property/decorators/local
		 ...
		 *
		 *  admin/jqadm/swpost/characteristic/property/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/swpost/characteristic/property/decorators/global
		 */
		return $this->createSubClient( 'swpost/characteristic/property/' . $type, $name );
	}
	...
	{
		/** admin/jqadm/swpost/characteristic/property/standard/subparts
		 * List of JQAdm sub-clients rendered within the swpost property section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/swpost/characteristic/property/standard/subparts', [] );
	}


	/**
	 * Returns the available swpost property types
	 *
	...
	{
		$excludes = array( 'package-length', 'package-height', 'package-width', 'package-weight' );
		$manager = \Aimeos\MShop::create( $this->getContext(), 'swpost/property/type' );

		$search = $manager->createSearch( true )->setSlice( 0, 10000 );
		$search->setConditions( $search->compare( '!=', 'swpost.property.type.code', $excludes ) );
		$search->setSortations( [$search->sort( '+', 'swpost.property.type.position' )] );
	...
	{
		$manager = \Aimeos\MShop::create( $this->getContext(), 'swpost/property' );
		$propItems = $this->excludeItems( $item->getPropertyItems( null, false ) );

		foreach( $data as $entry )
		{
			if( isset( $propItems[$entry['swpost.property.id']] ) )
			{
				$propItem = $propItems[$entry['swpost.property.id']];
				unset( $propItems[$entry['swpost.property.id']] );
			}
			else
			{
	...
	protected function toArray( \Aimeos\MShop\Swpost\Item\Iface $item, bool $copy = false ) : array
	{
		...
		foreach( $this->excludeItems( $item->getPropertyItems( null, false ) ) as $item )
		{
			...
			if( $copy === true )
			{
				$list['swpost.property.siteid'] = $siteId;
				$list['swpost.property.id'] = '';
			}
	...
	{
		/** admin/jqadm/swpost/characteristic/property/template-item
		 * Relative path to the HTML body template of the property characteristic subpart for swposts.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/swpost/characteristic/property/template-item';
		$default = 'swpost/item-characteristic-property-standard';
