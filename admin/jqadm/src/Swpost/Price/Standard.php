...
namespace Aimeos\Admin\JQAdm\Swpost\Price;

sprintf( 'price' ); // for translation


/**
 * Default implementation of swpost price JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/swpost/price/name
	 * Name of the price subpart used by the JQAdm swpost implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Swpost\Price\Myname".
	...
	public function addData( \Aimeos\MW\View\Iface $view ) : \Aimeos\MW\View\Iface
	{
		...
		$search->setConditions( $search->compare( '==', 'price.type.domain', 'swpost' ) );
		$search->setSortations( array( $search->sort( '+', 'price.type.position' ) ) );

		$listSearch = $listTypeManager->createSearch( true )->setSlice( 0, 10000 );
		$listSearch->setConditions( $listSearch->compare( '==', 'swpost.lists.type.domain', 'price' ) );
		$listSearch->setSortations( array( $listSearch->sort( '+', 'swpost.lists.type.position' ) ) );
	...
	{
		/** admin/jqadm/swpost/price/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/swpost/price/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/swpost/price/decorators/global
		 * @see admin/jqadm/swpost/price/decorators/local
		 */

		/** admin/jqadm/swpost/price/decorators/global
		 ...
		 *
		 *  admin/jqadm/swpost/price/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/swpost/price/decorators/excludes
		 * @see admin/jqadm/swpost/price/decorators/local
		 */

		/** admin/jqadm/swpost/price/decorators/local
		 ...
		 *
		 *  admin/jqadm/swpost/price/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/swpost/price/decorators/global
		 */
		return $this->createSubClient( 'swpost/price/' . $type, $name );
	}
	...
	{
		/** admin/jqadm/swpost/price/standard/subparts
		 * List of JQAdm sub-clients rendered within the swpost price section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/swpost/price/standard/subparts', [] );
	}
	 ...
	 *
	 * @param \Aimeos\MShop\Swpost\Item\Iface $item Swpost item including attribute items
	 * @return bool True if price value can be entered by the customer, false if not
	 */
	protected function isCustom( \Aimeos\MShop\Swpost\Item\Iface $item ) : bool
	{
	 ...
	 * @return \Aimeos\MShop\Swpost\Item\Iface $item Modified swpost item
	 */
	protected function setCustom( \Aimeos\MShop\Swpost\Item\Iface $item, $value ) : \Aimeos\MShop\Swpost\Item\Iface
	{
		$context = $this->getContext();
		...
		try
		{
			...
			$attrItem = $attrManager->findItem( 'custom', [], 'swpost', 'price' );
		}
		catch( \Aimeos\MShop\Exception $e )
		{
			$attrItem = $attrManager->createItem()->setDomain( 'swpost' )->setType( 'price' )->setCode( 'custom' );
		...
		if( $value )
		{
			...
			{
				$listItem = \Aimeos\MShop::create( $context, 'swpost' )->createListItem();
	...
	{
		/** admin/jqadm/swpost/price/template-item
		 * Relative path to the HTML body template of the price subpart for swposts.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/swpost/price/template-item';
		$default = 'swpost/item-price-standard';
