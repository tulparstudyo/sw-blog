...
namespace Aimeos\Admin\JQAdm\Post\Price;

sprintf( 'price' ); // for translation


/**
 * Default implementation of post price JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/post/price/name
	 * Name of the price subpart used by the JQAdm post implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Post\Price\Myname".
	...
	public function addData( \Aimeos\MW\View\Iface $view ) : \Aimeos\MW\View\Iface
	{
		...
		$search->setConditions( $search->compare( '==', 'price.type.domain', 'post' ) );
		$search->setSortations( array( $search->sort( '+', 'price.type.position' ) ) );

		$listSearch = $listTypeManager->createSearch( true )->setSlice( 0, 10000 );
		$listSearch->setConditions( $listSearch->compare( '==', 'post.lists.type.domain', 'price' ) );
		$listSearch->setSortations( array( $listSearch->sort( '+', 'post.lists.type.position' ) ) );
	...
	{
		/** admin/jqadm/post/price/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/post/price/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/post/price/decorators/global
		 * @see admin/jqadm/post/price/decorators/local
		 */

		/** admin/jqadm/post/price/decorators/global
		 ...
		 *
		 *  admin/jqadm/post/price/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/post/price/decorators/excludes
		 * @see admin/jqadm/post/price/decorators/local
		 */

		/** admin/jqadm/post/price/decorators/local
		 ...
		 *
		 *  admin/jqadm/post/price/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/post/price/decorators/global
		 */
		return $this->createSubClient( 'post/price/' . $type, $name );
	}
	...
	{
		/** admin/jqadm/post/price/standard/subparts
		 * List of JQAdm sub-clients rendered within the post price section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/post/price/standard/subparts', [] );
	}
	...
	protected function fromArray( \Aimeos\MShop\Post\Item\Iface $item, array $data ) : \Aimeos\MShop\Post\Item\Iface
	{
		...
		foreach( $data as $idx => $entry )
		{
			$listType = $entry['post.lists.type'];
	 ...
	 *
	 * @param \Aimeos\MShop\Post\Item\Iface $item Post item including attribute items
	 * @return bool True if price value can be entered by the customer, false if not
	 */
	protected function isCustom( \Aimeos\MShop\Post\Item\Iface $item ) : bool
	{
	 ...
	 * @return \Aimeos\MShop\Post\Item\Iface $item Modified post item
	 */
	protected function setCustom( \Aimeos\MShop\Post\Item\Iface $item, $value ) : \Aimeos\MShop\Post\Item\Iface
	{
		$context = $this->getContext();
		...
		try
		{
			...
			$attrItem = $attrManager->findItem( 'custom', [], 'post', 'price' );
		}
		catch( \Aimeos\MShop\Exception $e )
		{
			$attrItem = $attrManager->createItem()->setDomain( 'post' )->setType( 'price' )->setCode( 'custom' );
		...
		if( $value )
		{
			...
			{
				$listItem = \Aimeos\MShop::create( $context, 'post' )->createListItem();
	...
	{
		/** admin/jqadm/post/price/template-item
		 * Relative path to the HTML body template of the price subpart for posts.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/post/price/template-item';
		$default = 'post/item-price-standard';
