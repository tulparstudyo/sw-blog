...
namespace Aimeos\Admin\JQAdm\Post\Download;

sprintf( 'download' ); // for translation


/**
 * Default implementation of post download JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/post/download/name
	 * Name of the download subpart used by the JQAdm post implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Post\Download\Myname".
	...
	{
		$view = $this->getObject()->addData( $this->getView() );
		$data['post.lists.siteid'] = $this->getContext()->getLocale()->getSiteId();
	...
	{
		/** admin/jqadm/post/download/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/post/download/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/post/download/decorators/global
		 * @see admin/jqadm/post/download/decorators/local
		 */

		/** admin/jqadm/post/download/decorators/global
		 ...
		 *
		 *  admin/jqadm/post/download/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/post/download/decorators/excludes
		 * @see admin/jqadm/post/download/decorators/local
		 */

		/** admin/jqadm/post/download/decorators/local
		 ...
		 *
		 *  admin/jqadm/post/download/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/post/download/decorators/global
		 */
		return $this->createSubClient( 'post/download/' . $type, $name );
	}
	...
	{
		/** admin/jqadm/post/download/standard/subparts
		 * List of JQAdm sub-clients rendered within the post download section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/post/download/standard/subparts', [] );
	}
	...
	protected function fromArray( \Aimeos\MShop\Post\Item\Iface $item, array $data ) : \Aimeos\MShop\Post\Item\Iface
	{
		...
		$prodManager = \Aimeos\MShop::create( $context, 'post' );
		...
		{
			$listId = $this->getValue( $data, 'post.lists.id' );
	...
	protected function toArray( \Aimeos\MShop\Post\Item\Iface $item, bool $copy = false ) : array
	{
		...
		foreach( $item->getListItems( 'attribute', 'hidden', 'download', false ) as $listItem )
		{
			...
			catch( \Exception $e ) { ; } // Show post even if file isn't available any more
		}
	...
	{
		/** admin/jqadm/post/download/template-item
		 * Relative path to the HTML body template of the download subpart for posts.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/post/download/template-item';
		$default = 'post/item-download-standard';
