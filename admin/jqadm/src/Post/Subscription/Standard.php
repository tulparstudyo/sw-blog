...
namespace Aimeos\Admin\JQAdm\Post\Subscription;

sprintf( 'subscription' ); // for translation


/**
 * Default implementation of post subscription JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/post/subscription/name
	 * Name of the subscription subpart used by the JQAdm post implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Post\Subscription\Myname".
	...
	{
		/** admin/jqadm/post/subscription/decorators/excludes
		 * Excludes decorators added by the "common" subscription from the post JQAdm client
		 *
		 ...
		 *
		 *  admin/jqadm/post/subscription/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/post/subscription/decorators/global
		 * @see admin/jqadm/post/subscription/decorators/local
		 */

		/** admin/jqadm/post/subscription/decorators/global
		 ...
		 *
		 *  admin/jqadm/post/subscription/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/post/subscription/decorators/excludes
		 * @see admin/jqadm/post/subscription/decorators/local
		 */

		/** admin/jqadm/post/subscription/decorators/local
		 ...
		 *
		 *  admin/jqadm/post/subscription/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/post/subscription/decorators/global
		 */
		return $this->createSubClient( 'post/subscription/' . $type, $name );
	}
	...
	protected function getIntervalItems() : \Aimeos\Map
	{
		...
		$expr = [
			...
			$search->compare( '==', 'attribute.domain', 'post' ),
		];
	...
	{
		/** admin/jqadm/post/subscription/standard/subparts
		 * List of JQAdm sub-clients rendered within the post subscription section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/post/subscription/standard/subparts', [] );
	}
	...
	protected function toArray( \Aimeos\MShop\Post\Item\Iface $item, bool $copy = false ) : array
	{
		...
		foreach( $this->getIntervalItems() as $attrId => $attrItem )
		{
			...
			{
				$list['post.lists.siteid'] = (string) $map[$attrId]->getSiteId();
				$list['post.lists.id'] = (string) $map[$attrId]->getId();
			}
			else
			{
	...
	{
		/** admin/jqadm/post/subscription/template-item
		 * Relative path to the HTML body template of the subscription subpart for posts.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/post/subscription/template-item';
		$default = 'post/item-subscription-standard';
