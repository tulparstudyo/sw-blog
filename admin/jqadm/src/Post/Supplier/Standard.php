...
namespace Aimeos\Admin\JQAdm\Post\Supplier;

sprintf( 'supplier' ); // for translation


/**
 * Default implementation of post supplier JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/post/supplier/name
	 * Name of the supplier subpart used by the JQAdm post implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Post\Supplier\Myname".
	...
	public function delete() : ?string
	{
		...
		$expr = array(
			...
			$search->compare( '==', 'supplier.lists.domain', 'post' )
		);
	...
	{
		/** admin/jqadm/post/supplier/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/post/supplier/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/post/supplier/decorators/global
		 * @see admin/jqadm/post/supplier/decorators/local
		 */

		/** admin/jqadm/post/supplier/decorators/global
		 ...
		 *
		 *  admin/jqadm/post/supplier/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @supplier Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/post/supplier/decorators/excludes
		 * @see admin/jqadm/post/supplier/decorators/local
		 */

		/** admin/jqadm/post/supplier/decorators/local
		 ...
		 *
		 *  admin/jqadm/post/supplier/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/post/supplier/decorators/global
		 */
		return $this->createSubClient( 'post/supplier/' . $type, $name );
	}
	...
	{
		/** admin/jqadm/post/supplier/standard/subparts
		 * List of JQAdm sub-clients rendered within the post supplier section
		 *
		 ...
		 * @supplier Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/post/supplier/standard/subparts', [] );
	}
	...
	/**
	 * Returns the supplier list items for the given post ID
	 *
	...
	protected function fromArray( \Aimeos\MShop\Post\Item\Iface $item, array $data )
	{
		...
		foreach( $data as $idx => $entry )
		{
			...
			$list[] = $litem->setParentId( $this->getValue( $entry, 'supplier.id' ) )->setDomain( 'post' )
	...
	{
		/** admin/jqadm/post/supplier/template-item
		 * Relative path to the HTML body template of the supplier subpart for posts.
		 *
		 ...
		 * @supplier Developer
		 */
		$tplconf = 'admin/jqadm/post/supplier/template-item';
		$default = 'post/item-supplier-standard';
