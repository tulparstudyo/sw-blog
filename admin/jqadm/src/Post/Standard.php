	...
	implements \Aimeos\Admin\JQAdm\Common\Admin\Factory\Iface
{
	...
	public function create() : ?string
	{
		...
		try
		{
			...
			if( !isset( $view->item ) ) {
				$view->item = \Aimeos\MShop::create( $this->getContext(), 'post' )->createItem();
			}
	...
	public function save() : ?string
	{
		...
		try
		{
			...
			$context->getCache()->deleteByTags( ['post', 'post-' . $item->getId()] );
	...
	public function search() : ?string
	{
		...
		{
			$total = 0;
			$domains = map( $this->getDomains() )->remove( 'post' );
			$params = $this->storeSearchParams( $view->param(), 'post' );
			$manager = \Aimeos\MShop::create( $this->getContext(), 'post' );

			$search = $manager->createSearch();
			$search->setSortations( [$search->sort( '+', 'post.id' )] );
	...
	protected function getDomains() : array
	{
		 ...
		 * list of domains (attribute, media, price, post, text, etc. are
	...
	{
		/** admin/jqadm/post/standard/subparts
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/post/standard/subparts', ['text', 'media'] );
	}


	/**
	 * Returns the available post type items
	 *
	 * @return \Aimeos\Map List of item implementing \Aimeos\MShop\Common\Type\Iface
	 */
	protected function getTypeItems() : \Aimeos\Map
	{
		$typeManager = \Aimeos\MShop::create( $this->getContext(), 'post/type' );

		$search = $typeManager->createSearch( true )->setSlice( 0, 10000 );
		$search->setSortations( [$search->sort( '+', 'post.type.position' )] );
	...
	protected function fromArray( array $data ) : \Aimeos\MShop\Post\Item\Iface
	{
		...
		if( isset( $data['post.id'] ) && $data['post.id'] != '' ) {
			$item = $manager->getItem( $data['post.id'], $this->getDomains() );
		} else {
	...
	protected function toArray( \Aimeos\MShop\Post\Item\Iface $item, bool $copy = false ) : array
	{
		...
		if( $copy === true )
		{
			...
			$data['post.code'] = $data['post.code'] . '_copy';
			$data['post.url'] = $data['post.url'] . '-' . time();
			$data['post.id'] = '';
		}
