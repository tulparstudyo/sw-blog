...
namespace Aimeos\Admin\JQAdm\Post\Selection;

sprintf( 'selection' ); // for translation


/**
 * Default implementation of post selection JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/post/selection/name
	 * Name of the selection subpart used by the JQAdm post implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Post\Selection\Myname".
	...
	{
		/** admin/jqadm/post/selection/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/post/selection/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/post/selection/decorators/global
		 * @see admin/jqadm/post/selection/decorators/local
		 */

		/** admin/jqadm/post/selection/decorators/global
		 ...
		 *
		 *  admin/jqadm/post/selection/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/post/selection/decorators/excludes
		 * @see admin/jqadm/post/selection/decorators/local
		 */

		/** admin/jqadm/post/selection/decorators/local
		 ...
		 *
		 *  admin/jqadm/post/selection/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/post/selection/decorators/global
		 */
		return $this->createSubClient( 'post/selection/' . $type, $name );
	}
	...
	{
		/** admin/jqadm/post/selection/standard/subparts
		 * List of JQAdm sub-clients rendered within the post selection section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/post/selection/standard/subparts', [] );
	}
	...
	protected function fromArray( \Aimeos\MShop\Post\Item\Iface $item, array $data ) : \Aimeos\MShop\Post\Item\Iface
	{
		...
		$listItems = $item->getListItems( 'post', 'default', null, false );
		$codes = [];

		foreach( $data as $idx => $entry )
		{
			if( ( $litem = $item->getListItem( 'post', 'default', $entry['post.id'], false ) ) === null ) {
			...
			$item->addListItem( 'post', $litem, $refItem );
	...
	/**
	 * Updates the variant attributes of the given post item
	 *
	 * @param \Aimeos\MShop\Post\Item\Iface $refItem Article item object
	 * @param array $entry Associative list of key/values for post attribute references
	 * @return \Aimeos\MShop\Post\Item\Iface Updated artice item object
	 */
	protected function fromArrayAttributes( \Aimeos\MShop\Post\Item\Iface $refItem, array $entry )
	{
		$listManager = \Aimeos\MShop::create( $this->getContext(), 'post/lists' );
		$litems = $refItem->getListItems( 'attribute', 'variant', null, false );
		$pos = 0;

		foreach( $entry as $attr )
		{
			if( !isset( $attr['post.lists.refid'] ) || $attr['post.lists.refid'] == '' ) {
				continue;
			}

			if( ( $litem = $refItem->getListItem( 'attribute', 'variant', $attr['post.lists.refid'], false ) ) === null ) {
	...
	/**
	 * Updates the stocklevels for the posts
	 *
	 * @param array $data List of post codes
	 * @param array $data Data array
	 */
	protected function fromArrayStocks( array $codes, array $data )
	{
		...
		$map = $stockItems->col( 'stock.postcode', 'stock.id' );
		$list = [];
		...
		foreach( $data as $entry )
		{
			...
			$code = $entry['post.code'];

			if( ( $stockItem = $stockItems->get( $map[$code] ?? null ) ) === null ) {
				$stockItem = $manager->createItem();
			}

			$stockItem->fromArray( $entry, true )->setPostCode( $code )->setType( 'default' );
	...
	protected function toArray( \Aimeos\MShop\Post\Item\Iface $item, bool $copy = false ) : array
	{
		...
		foreach( $item->getListItems( 'post', 'default', null, false ) as $id => $listItem )
		{
	...
	{
		/** admin/jqadm/post/selection/template-item
		 * Relative path to the HTML body template of the selection subpart for posts.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/post/selection/template-item';
		$default = 'post/item-selection-standard';
